...
const { createPost: createPostDb,
  updatePost: updatePostDb,
  deletePost: deletePostDb,
} = postServiceDB
export async function createPost(unsafeData: z.infer<typeof PostDetailsSchema>): Promise<{
  error: boolean;
  message: string;
} | undefined> {
...
export async function updatePost(id: string, unsafeData: z.infer<typeof PostDetailsSchema>): Promise<{
  error: boolean;
  message: string;
} | undefined> {
  const session = await auth()
  ...
  return {
    error: !isSuccess,
    message: isSuccess ? "Post updated successfully" : "There was an error updating the post",
  }
}

export async function deletePost(id: string, userId: string): Promise<{
  error: boolean;
  message: string;
} | undefined> {
  const session = await auth()
  ...
  return {
    error: !isSuccess,
    message: isSuccess ? "Post deleted successfully" : "There was an error deleting the post",
  }
}
